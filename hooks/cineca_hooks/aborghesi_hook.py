...
if pbs.event().type == pbs.RUNJOB:
    try:
        ...
        # if any exec node is not in the whitelist, do not create temporary dir
        if all_nodes_whitelist != 0 and is_job_array != 1:
            # write info in temporary file file
            tmpdir = templogs_dir + job_id 
            json_file = tmpdir + '/job_request_info.json'

            if not os.path.exists(tmpdir):
                os.makedirs(tmpdir)

            with open(json_file, 'w') as fp:
                json.dump(job_dict, fp, sort_keys=True, indent=4)
    ...
    try:
        ...
        # write info in temporary file file
        tmpdir = templogs_dir + job_id 
        json_file = tmpdir + '/job_begin_info_' + str(node_id) + '.json'

        # we create BEGIN tmp file only if job_request was previously created 
    ...
    try:
        ...
        # write info in temporary file file
        tmpdir = templogs_dir + job_id 
        json_file = tmpdir + '/job_end_info_' + str(node_id) + '.json'

        # we create END tmp file only if job_request was previously created 
        if os.path.exists(tmpdir):
            #os.makedirs(tmpdir)
            with open(json_file, 'w') as fp:
                json.dump(job_dict, fp, sort_keys=True, indent=4)
